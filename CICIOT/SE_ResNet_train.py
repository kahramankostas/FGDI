...
from SE_ResNet_model import se_resnet34
...
epochs = 20
save_path = './SE_ResNet_parameters.pth'
best_f1 = 0.0
train_accurate_list = []
val_accurate_list = []
f1_list = []
...
for epoch in range(epochs):
    ...
    with open("SE_ResNet_result_npy.txt", 'a') as file:
...
torch.save(pre_last, 'pre_val_label/pre_last_SE_ResNet.pt')
torch.save(val_last, 'pre_val_label/val_last_SE_ResNet.pt')
# 迭代次数列表作为横坐标
iterations = range(1, len(train_accurate_list) + 1)
with open("SE_ResNet_npy_plt_data.txt", 'a') as file:
...
plot_accuracy(iterations, train_accurate_list, val_accurate_list, save_path='SE_ResNet_accuracy_plot_npy.png')
plot_f1_score(iterations, f1_list, save_path='SE_ResNet_f1_score_plot_npy.png')
conf_matrix = confusion_matrix(val_last.cpu(),pre_last.cpu())
plot_matrix(conf_matrix,dev_list,"SE_ResNet_confusion_matrix_npy.png")
